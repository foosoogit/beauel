...
class CustomersList extends Component
{
	...
	public function sort($sort_key){
		...
		session(['sort_key_user' =>$sort_key_array[0]]);
		session(['asc_desc_user' =>$sort_key_array[1]]);
	}
    public function render()
    {
		...
		if(!isset($sort_key_p) and session('sort_key')==null){
			session(['sort_key_user' =>'']);
		}
		$this->sort_key_p=session('sort_key_user');

		if(!isset($asc_desc_p) and session('asc_desc_user')==null){
			session(['asc_desc_user' =>'ASC']);
		}
		$this->asc_desc_p=session('asc_desc_user');
		...
		$userQuery=$userQuery->where('branch',session('target_branch_serial'));
		...
		*/
		$targetSortKey="";
		if(session('sort_key_user')<>""){
			$targetSortKey=session('sort_key_user');
		}else{
