...
class PaymentHistoryList extends Component
{
    ...
    public function render()
    {
		...
		$PaymentHistoryQuery =$PaymentHistoryQuery->where('branch',session('target_branch_serial'));
		...
		if($UserSerial==0){
			//$newPaymentHistorySerial=PaymentHistory::where('serial_user','=',session('target_user_serial_sub'))->max('payment_history_serial');
			$newPaymentHistorySerial=PaymentHistory::where('serial_user','=',session('target_user_serial'))->max('payment_history_serial');
			if(empty($newPaymentHistorySerial)){
				//$newPaymentHistorySerial='P_'.session('target_user_serial_sub').'-0000-01';
				$newPaymentHistorySerial='P_'.session('target_user_serial').'-0000-01';
			}else{
				$newPaymentHistorySerial++;
			}
			//$UserInf=User::where('serial_user','=',session('target_user_serial_sub'))->first();
			$UserInf=User::where('serial_user','=',session('target_user_serial'))->first();
			if($this->kensakukey<>""){
				$this->dynamic_key="%".$this->kensakukey."%";
				$PaymentHistoryQuery=$PaymentHistoryQuery->where('serial_user','=',session('target_user_serial'))
					->where(function($q){
