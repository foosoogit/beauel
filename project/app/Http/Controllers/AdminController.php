...
class AdminController extends Controller
{
	...
	public function __construct(){
		...
		public function ajax_upsert_CashBookSmall(Request $request){
        $rec = [
            [
				'id' => $request->id,
        ...
        CashBooksSmall::upsert($rec, ['id']);
		if($request->id==""){
			$Tid=CashBooksSmall::where('branch', session('target_branch_serial'))->max('id');
		}else{
			$Tid=$request->id;
		}
		OtherFunc::set_target_balance_to_CashBookSmallDb($Tid);
	}
	...
	public function ShowInpRecordVisitPayment($ContractSerial,$UserSerial){
		...
		$html_payment_record_table=OtherFunc::make_html_payment_record_table($ContractSerial,$UserSerial);
		...
		if($paymentCount<$targetContract->how_many_pay_card){
			$paymentCount=$targetContract->how_many_pay_card;
		}
		if($paymentCount<$targetContract->payments_num){
		...
		return view('customers.PaymentRegistration',compact("html_payment_record_table","UserSerial","PointArray","target_historyBack_inf_array","only_treatment_color_array","GoBackToPlace",'VisitSerialArray','VisitDateArray','PaymentDateArray','targetUser','targetContract','KeiyakuNaiyou','PaymentAmountArray','HowToPayCheckedArray','visit_disabeled','sejyutukaisu','set_gray_array','payment_disabeled','set_gray_pay_array','set_background_gray_pay_array','paymentCount','TreatmentDetailsArray','TreatmentDetailsSelectArray'));
	}

	public function CashBookRep_download(Request $request){
        $spreadsheet = new Spreadsheet();
        $start_Period=$request->year_start.'-'.sprintf('%02d', $request->month_start).'-00';
		$end_Period=$request->year_end.'-'.sprintf('%02d',$request->month_end).'-31';

        $CashBook_inf=CashBook::where('target_date','>=',$start_Period)
			->where('target_date','<=',$end_Period)
			->where('branch',session('target_branch_serial'))->get();
        $sheetNum=0;$retu=1;$gyou=0;$tYM_old="";
        foreach($CashBook_inf as $CashBook_res){
			$tdat=$CashBook_res->target_date;
            $YMD_array=explode("-", $CashBook_res->target_date);
			$tYM=trim($YMD_array[0]).trim($YMD_array[1]);
			if($tYM<>$tYM_old){
				if($sheetNum==0){
				...
				$tmnt=$YMD_array[1];
				if($tmnt<>session('target_mnt')){
					session(['target_mnt' =>$tmnt]);
					$sheet->setTitle($YMD_array[0].'-'.$YMD_array[1]);
				}
			}
			if($gyou==1){
				$sheet->setTitle($tYM);
				$sheet->setCellValue([$retu,$gyou], $YMD_array[0].'年'.$YMD_array[1].'月');
				$sheet->setCellValue([$retu,$gyou+1], 'シリアル');
            	$sheet->setCellValue([$retu+1,$gyou+1], '日付');
				$sheet->setCellValue([$retu+2,$gyou+1], '摘要');
				$sheet->setCellValue([$retu+3,$gyou+1], '出金');
				$sheet->setCellValue([$retu+4,$gyou+1], '入金');
				$sheet->setCellValue([$retu+5,$gyou+1], '備考');
				for($cnt=0;$cnt<=5;$cnt++){
					$sheet->getStyle( [$retu+$cnt,$gyou+1] )->getAlignment()->setHorizontal('center');  // 中央寄せ
				}
				$gyou=$gyou+1;
			}
			$sheet->setCellValue([$retu,$gyou+1], $CashBook_res->id);
			//$sheet->setCellValue([$retu+1,$gyou+1], $YMD_array[1].'月'.$YMD_array[2].'日('.date('w', strtotime($CashBook_res->target_date)).')');
			$sheet->setCellValue([$retu+1,$gyou+1], $YMD_array[1].'月'.$YMD_array[2].'日('.OtherFunc::day_of_the_week_dtcls(date('w', strtotime($CashBook_res->target_date))).')');
			$sheet->setCellValue([$retu+2,$gyou+1], $CashBook_res->summary);
			$sheet->setCellValue([$retu+3,$gyou+1], $CashBook_res->payment);
			$sheet->getStyle([$retu+3,$gyou+1])->getNumberFormat()->setFormatCode('#,##0');
			$sheet->setCellValue([$retu+4,$gyou+1], $CashBook_res->deposit);
			$sheet->getStyle([$retu+4,$gyou+1])->getNumberFormat()->setFormatCode('#,##0');
			$sheet->setCellValue([$retu+5,$gyou+1], $CashBook_res->remarks);
			$gyou++;
        ...
        $fileName = '出納帳_'.$request->year_start.sprintf('%02d', $request->month_start).'～'.$request->year_end.sprintf('%02d',$request->month_end).'.xlsx';
	...
	public function ShowMenuCustomerManagement(Request $request){
		if(empty($request->target_branch)){
			if(OtherFunc::select_branch_ck(session('target_branch_serial'))<=0){
				return redirect()->route('admin.select_branch');
			}
		}else{
			session(['target_branch_serial'=>$request->target_branch]);
		}
		$target_branch_inf=Branch::select('name_branch')->where('serial_branch',session('target_branch_serial'))->first();
		session(['target_branch_name' => $target_branch_inf->name_branch]);
	...
	public function ShowSyuseiContract($ContractSerial,$UserSerial){
		...
		$select_copamny_dispnone="style='display:none'";
		if($targetContract->how_to_pay=="card"){
			$select_copamny_dispnone="";
		}

		$html_how_to_pay=OtherFunc::make_html_how_to_pay_slct($targetContract->how_to_pay);
		$html_payments_num_slct=OtherFunc::make_html_payments_num_slct($targetContract->payments_num);
		$GoBackPlace=$_SESSION['access_history'][0];
		return view('customers.CreateContract',compact("select_copamny_dispnone","html_payments_num_slct","html_how_to_pay","contract_type_checked","html_staff_slct",'newKeiyakuSerial','targetContract',"targetContractdetails","targetUser","KeiyakuNaiyouArray","KeiyakuNumSlctArray","KeiyakuTankaArray","KeiyakuPriceArray","HowToPay","HowManyPay","CardCompanySelect","GoBackPlace","TreatmentsTimes_slct","KeiyakuNaiyouSelectArray"));
	}
	...
	public function ShowInpKeiyaku($serial_user){
		...
		$html_how_to_pay=OtherFunc::make_html_how_to_pay_slct("");
		...
		$contract_type_checked['cyclic']='';
		$select_copamny_dispnone="style='display:none'";
		$html_payments_num_slct=OtherFunc::make_html_payments_num_slct("");
		return view('customers.CreateContract',compact("select_copamny_dispnone","html_payments_num_slct","html_how_to_pay","contract_type_checked","html_staff_slct","targetUser","header","slot","tday","endDay","newKeiyakuSerial","targetContract","KeiyakuNaiyouArray","KeiyakuNumSlctArray","KeiyakuTankaArray","KeiyakuPriceArray","HowToPay","CardCompanySelect","HowManyPay",'TreatmentsTimes_slct','KeiyakuNaiyouSelectArray'));
	}
	...
	public function ShowPaymentRegistrationIflame($SerialKeiyaku,$SerialUser){
		...
		$target_historyBack_inf_array=initConsts::TargetPageInf($_SESSION['access_history'][0]);
		$KeiyakuInf="";
		//Log::alert("SerialKeiyaku=".$SerialKeiyaku);
		//Log::alert("SerialUser=".$SerialUser);
		session(['target_user_serial_sub'=>$SerialUser]);
		session(['target_user_serial'=>$SerialUser]);
		if($SerialKeiyaku<>"0"){
