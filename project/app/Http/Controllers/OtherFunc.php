...
{
	/*
	public static function html_make_select_summary_small_ajax(){
	...
	*/
	...
	public static function set_target_balance_to_CashBookSmallDb($target_id){
		...
		$fnd_flg=false;	
		$CashBook_inf_array=CashBooksSmall::where('branch',session('target_branch_serial'))->orderBy('target_date', 'asc')->orderBy('created_at', 'asc')->get();
		foreach($CashBook_inf_array as $CashBook_inf){
			if($CashBook_inf->id==$target_id or $fnd_flg==true){
				...
				$target_rec = CashBooksSmall::find($CashBook_inf->id);
	...
	public static function make_html_payment_record_table($ContractSerial,$UserSerial){
		$targetContract=Contract::where('serial_keiyaku','=', $ContractSerial)->first();
		$paymentCount=$targetContract->payments_num;
		$targetPaymentHistoryArray=PaymentHistory::where('serial_keiyaku','=', $ContractSerial)->get();
		$htm_payment_record_table='<table style="width: 100%"><tr><td colspan="12">支払い記録';
		if ($targetContract->keiyaku_type<>'subscription'){
			$htm_payment_record_table.='（支払い回数：'.$paymentCount.'回）';
		}
		$htm_payment_record_table.='</td></tr>';
		$htm_payment_record_table.='<tr>';
		$HowToPayCheckedArray=array();
		$set_background_gray_pay_array = array_fill(0, initConsts::PaymentNumMax(),'');
		$payment_disabeled_array=array_fill(0, initConsts::PaymentNumMax(),'');
		for($i=0;$i<initConsts::PaymentNumMax();$i++){
			...
			if($targetContract->keiyaku_type=="subscription"){
				...
				$payment_disabeled_array[$i]='disabled';
				$set_gray_pay_array[$i]='color:#DDDDDD';$set_background_gray_pay_array[$i]='background-color:#EEEEEE';
				$set_gray_pay='color:#DDDDDD';
				if($i<$paymentCount){
					$payment_disabeled_array[$i]="";
					$set_gray_pay_array[$i]="";
					$set_background_gray_pay_array[$i]='';
					$set_gray_pay='';
				}
				$htm_payment_record_table.='<td style="text-align: center;'.$set_gray_pay.'">'.strval($i+1).'ヶ月目</td>';	
			}	
		}
		$htm_payment_record_table.='</tr><tr>';
		$PaymentAmountArray=array();
		$PaymentDateArray=array();
		$how_to_pay_array=array();
		foreach($targetPaymentHistoryArray as $targetPaymentHistory){
			...
			$how_to_pay_array[]=$targetPaymentHistory->how_to_pay;
		}
		for($i=0;$i<initConsts::PaymentNumMax();$i++){
			//$set_background_gray_pay='background-color:#EEEEEE';
			//if($i<$paymentCount){$set_background_gray_pay='';}
			if(isset($PaymentAmountArray[$i])){
				$htm_payment_record_table.='<td><input name="PaymentAmount['.$i.']" type="text"  style="text-align: right;width: 100%;'.$set_background_gray_pay_array[$i].'" value="'.$PaymentAmountArray[$i].'" '.$payment_disabeled_array[$i].'/></td>';
			}else{
				$htm_payment_record_table.='<td><input name="PaymentAmount['.$i.']" type="text"  style="text-align: right;width: 100%;'.$set_background_gray_pay_array[$i].'" value=""'.$payment_disabeled_array[$i].'/></td>';
			}
		...
		$htm_payment_record_table.='</tr><tr>';
		for($i=0;$i<initConsts::PaymentNumMax();$i++){
			if(isset($PaymentDateArray[$i])){
				$htm_payment_record_table.='<td><input name="PaymentDate['.$i.']" type="date" value="'.$PaymentDateArray[$i].'" style="'.$set_gray_pay_array[$i].'"'.$payment_disabeled_array[$i].'/></td>';
				//$htm_payment_record_table.='<td><input name="PaymentAmount['.$i.']" type="text"  style="text-align: right;width: 100%;'.$set_background_gray_pay_array[$i].'" value="{{optional('.$PaymentAmountArray[$i].'"'.$payment_disabeled_array[$i].'/></td>';
			}else{
				$htm_payment_record_table.='<td><input name="PaymentDate['.$i.']" type="date" value="" style="'.$set_gray_pay_array[$i].'"'.$payment_disabeled_array[$i].'/></td>';
				//$htm_payment_record_table.='<td><input name="PaymentAmount['.$i.']" type="text"  style="text-align: right;width: 100%;'.$set_background_gray_pay_array[$i].'" value=""'.$payment_disabeled_array[$i].'/></td>';
			}
		...
		*/
		...
		for($i=0;$i<initConsts::PaymentNumMax();$i++){
			$cnt=$i+1;
			$htm_payment_record_table.='<td style="'.$set_gray_pay_array[$i].'">';
			$htm_payment_record_table.='<select name="HowToPay['.$i.']" '.$payment_disabeled_array[$i].'>';
			$htm_payment_record_table.='<option value="">--選択してください。--</option>';
			...
			foreach($PaymentMethodArray as $PaymentMethod){
				$Method_array=explode("_", $PaymentMethod);
				//$htm_payment_record_table.='<td class="border px-4 py-2" style="text-align: right;">';
				$sctd_flg=false;
				$sct="";
				if(isset($how_to_pay_array[$i])){
					if($how_to_pay_array[$i]==$Method_array[0]){
						//Log::alert("Method_array=".$Method_array[0]);
						//Log::alert("how_to_pay_array=".$how_to_pay_array[$i]);
						$sct='Selected="selected"';
						$sctd_flg=true;
					}
				}
				if($sctd_flg){
					$htm_payment_record_table.='<option '.$sct.' value="'.$Method_array[0].'" >'.$Method_array[1].'</option>';
				}else{
					$htm_payment_record_table.='<option value="'.$Method_array[0].'">'.$Method_array[1].'</option>';	
				}
				//Log::alert("option=".'<option '.$sct.' value="'.$Method_array[0].'" >'.$Method_array[1].'</option>');
				//$htm_dayly_table.='</td>';
				//$cnt++;
			}
			//$htm_payment_record_table.='<option value="select-1" selected>selected</option>';	
			$htm_payment_record_table.='</select></td>';

			//$htm_payment_record_table.='<label style="'.$set_gray_pay_array[$i].'"><input name="HowToPay['.$i.']" type="radio" value="card" onclick="radioDeselection(this,'.$cnt.')" {!!optional($HowToPayCheckedArray[0])[0]!!}  {{$payment_disabeled[0]}}/>カード</label>';
		}
		/*
		...
		*/
		$htm_payment_record_table.='</tr></table>';
		return $htm_payment_record_table;
	
	}
	public static function make_html_dayly_report_table($today){                        
		$PaymentHistories=PaymentHistory::where('date_payment','=',$today)
            ->where('payment_histories.branch','=',session('target_branch_serial'))
            ->leftJoin('users', 'payment_histories.serial_user', '=', 'users.serial_user')->get();
		...
		*/
		...
		$htm_dayly_table='<table class="table-auto table-striped" border-solid><thead><tr><th class="border px-4 py-2">顧客No.</th><th class="border px-4 py-2 col-2 w-50">氏　　　　　名</th>';
		//$htm_dayly_table.='</tr></thead><tbody>';
		$Sum=array();
        $Sum['total']=0;
		$PaymentMethod=initConsts::PaymentMethod();
		$PaymentMethodArray=explode(",", $PaymentMethod);
        foreach($PaymentMethodArray as $value){
			...
			//$htm_dayly_table.='<th class="border px-4 py-2">'.$PaymentMethod_inf_array[0].'</th>';
            $Sum[$PaymentMethod_inf_array[0]]=PaymentHistory::where('date_payment','=',$today)
                ...
                ->where('payment_histories.branch','=',session('target_branch_serial'))
                ->sum('amount_payment');
			$htm_dayly_table.='<th class="border px-4 py-2">'.$PaymentMethod_inf_array[1].'</th>';
            //$Sum['total']=$Sum['total']+$Sum[$PaymentMethod_inf_array[0]];
		}
		Log::info($PaymentMethodArray);
        //$htm_dayly_table.='<th class="border px-4 py-2">合計</th></tr></thead><tbody><tr>';
		$payment_method_total=array_fill(0,count($PaymentMethod_key_array),0);
		$htm_dayly_table.='</tr></thead><tbody><tr>';
		foreach ($PaymentHistories as $PaymentHistory){
            $htm_dayly_table.='<td class="border px-4 py-2">'.$PaymentHistory->serial_user.'<br></td><td class="border px-4 py-2">';
            if($PaymentHistory->ContractType=='subscription'){
                $htm_dayly_table.='<form  method="GET" action="/customers/ShowPaymentRegistrationIflame/'.$PaymentHistory->serial_keiyaku.'/'.$PaymentHistory->serial_user.'"><x-csrf />';
                $htm_dayly_table.='<button type="submit" name="btn_serial" value="'.$PaymentHistory->serial_user.'">'.$PaymentHistory->name_sei.' '.$PaymentHistory->name_mei.'</button></form>';
			}else{
                $htm_dayly_table.='<form method="GET" action="/customers/ShowInpRecordVisitPayment/'.$PaymentHistory->serial_keiyaku.'/'.$PaymentHistory->serial_user.'"><x-csrf />';
                $htm_dayly_table.='<button type="submit" name="btn_serial" value="'.$PaymentHistory->serial_user.'">'.$PaymentHistory->name_sei.' '.$PaymentHistory->name_mei.'</button>';
                $htm_dayly_table.='<input name="target_day" type="hidden" value="'.$today.'"/></form></td>';
			}	
			//$htm_dayly_table.='<td class="border px-4 py-2">&nbsp;</td>';
			$pmCnt=0;
			//Log::alert("serial_user=".$PaymentHistory->serial_user);
			//Log::alert("how_to_pay=".$PaymentHistory->how_to_pay);
			//$payment_method_total=array();
			
			$cnt=0;
			foreach($PaymentMethod_key_array as $key){
				$htm_dayly_table.='<td class="border px-4 py-2" style="text-align: right;">';
                //Log::alert("how_to_pay=".$PaymentHistory->how_to_pay);
				if($PaymentHistory->how_to_pay==$key){
                    //Log::alert("key=".$key);
					//Log::alert("amount_payment=".$PaymentHistory->amount_payment);
					//$htm_dayly_table.=number_format($Sum[$PaymentMethod_inf_array[0]]);
					$htm_dayly_table.=number_format($PaymentHistory->amount_payment);
					Log::alert("key=".$key);
					Log::alert("amount=".$PaymentHistory->amount_payment);
					$payment_method_total[$cnt]=$payment_method_total[$cnt]+$PaymentHistory->amount_payment;
				}
				$htm_dayly_table.='</td>';
				$cnt++;
			}
			Log::alert("現金=".$payment_method_total[10]);
                /*
				$htm_dayly_table.='</td><td class="border px-4 py-2" style="text-align: right;">';
                    @if($PaymentHistory->how_to_pay=="")
		...
		$htm_dayly_table.='<td class="border px-4 py-2" style="text-align: right;" colspan="2">合計</td>';
		//$cnt=0;
		$total_all=0;
		for($i=0;$i<count($PaymentMethod_key_array);$i++){
		//foreach($PaymentMethod_key_array as $key){
			$htm_dayly_table.='<td class="border px-4 py-2" style="text-align: right;">'.number_format($payment_method_total[$i]).'円</td>';
			$total_all=$total_all+$payment_method_total[$i];
			//$cnt++;
		}
		$htm_dayly_table.='</tbody></table></table>';
		//$htm_dayly_table.='</tbody></table><td class="border px-4 py-2" style="text-align: right;">'.number_format($Sum['total']).'</td></tr></tbody></table>';
		return $htm_dayly_table;
	}
	...
	public static function make_html_monthly_report_table($targetYear,$targetMonth){
		$sbj_array=array();
		$PaymentMethod_hyouji_array=array();
		$PaymentMethod_key_array=array();
		$htm_month_table="";
		$sbj_array=['日'];
		$PaymentMethod=initConsts::PaymentMethod();
		$PaymentMethodArray=explode(",", $PaymentMethod);
		...
		if(session('target_branch_serial')=='01'){
			//$sbj_array=['日']
			//$PaymentMethod_hyouji_array
			array_push($sbj_array,$PaymentMethod_hyouji_array);
			array_push($PaymentMethod_hyouji_array,'合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数');
			//array_push($sbj_array,'合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数');
			//$sbj_array=['日','ｸﾚｼﾞｯﾄ・ﾛｰﾝ','PayPay','スマート支払い','現金','合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数'];	
		}else{
			array_push($PaymentMethod_hyouji_array,'合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数','契約<br>人数','累計<br>契約数','契約率');
			//array_push($sbj_array,'合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数','契約<br>人数','累計<br>契約数','契約率');
			//$sbj_array=['日','ｸﾚｼﾞｯﾄ・ﾛｰﾝ','PayPay','スマート支払い','現金','合計売上','累計売上','新規<br>来店数','会員<br>来店数','来店<br>合計','累計<br>来店数','契約<br>人数','累計<br>契約数','契約率'];
		}
		*/
		$htm_month_table='<table class="table-auto" border-solid><thead class="sticky-top">';
		$clm_cnt=1;
		foreach($sbj_array as $value){
			if($clm_cnt>$show_clm_cnt){
				$htm_month_table.='<td class="border px-4 py-2 not_show_cml" style="display:none">';
			}else{
				$htm_month_table.='<td class="border px-4 py-2">';
			}
			$htm_month_table.=$value.
		...
		$total_amount= array_fill(0 , count($PaymentMethodArray) ,0);
		$amount= array_fill(0 , count($PaymentMethodArray) ,0);
		...
		foreach($daterange as $date){
			...
			for($i=0;$i<count($PaymentMethodArray);$i++){
				$amount[$i]=self::get_uriage($date->format("Y-m-d"),$PaymentMethod_key_array[$i],'');
				$total_amount[$i]=$total_amount[$i]+$amount[$i];
			}
			...
			*/
			...
			for($mcnt=0;$mcnt<count($PaymentMethodArray);$mcnt++){
				$htm_month_table.='<td class="border px-4 py-2" style="text-align: right;">'.number_format($amount[$mcnt]).'</td>';
			...
			$htm_month_table.='<td class="border px-4 py-2" style="text-align: right;">'.number_format(session('TotalSales')).'</td>
					<td class="border px-4 py-2" style="text-align:">'.number_format(session('TotalSalesRuikei')).'</td>
					<td class="border px-4 py-2 not_show_cml" style="text-align: right;display:none;">'.number_format($new_visiters_cnt).'</td>
		...
		for($i=0;$i<count($PaymentMethodArray);$i++){
			$htm_month_table.='<td class="border px-4 py-2" style="text-align: right;">'.number_format($total_amount[$i]).'</td>';
		}
		/*
		...
		*/
		$htm_month_table.='
			...
			<td class="border px-4 py-2" style="text-align: right">'.number_format(session('TotalSalesRuikei')).'</td>
			<td class="border px-4 py-2 not_show_cml" style="text-align: right;display:none;">'.number_format($total_new_visiters_cnt).'</td>
	...
	public static function html_make_select_summary_ajax(){
		//console::log('target_branch_serial='+session('target_branch_serial'));
	...
	public static function select_branch_ck($target_branch_serial){
		$serial_branch_cnt=Branch::where('serial_branch',session('target_branch_serial'))->count();
		return $serial_branch_cnt;
	}

	public static function make_html_payments_num_slct($target_num){
		$htm_payments_num_slct="";
		$htm_payments_num_slct='<select name="payments_num_slct" id="payments_num_slct" class="form-select">';
		$sct='';
		if($target_num==''){
			$sct='Selected';
		}
		$htm_payments_num_slct.='<option value=0 '.$sct.'>-- 選択してください。 --</option>';
		for($i=1;$i<initConsts::PaymentNumMax();$i++){
			$sct='';
			if($target_num==$i){
				$sct='Selected';
			}
			$htm_payments_num_slct.='<option value="'.$i.'" '.$sct.' >'.$i.'</option>';
			$sct='';
		}
		$htm_payments_num_slct.='</select>';
		return $htm_payments_num_slct;
	}

	public static function make_html_how_to_pay_slct($target_serial){
		$PaymentMethod=initConsts::PaymentMethod();
		$PaymentMethodArray=explode(",", $PaymentMethod);
		$htm_PaymentMethod_slct="";
		$htm_PaymentMethod_slct='<select name="method_how_to_pay_slct" id="method_how_to_pay_slct" class="form-select">';
		$sct='';
		if($target_serial==''){
			$sct='Selected';
		}
		...
		foreach($PaymentMethodArray as $value){
			...
			$sct='';
			if(str_contains($value, $target_serial) && $target_serial!=''){
				$sct='Selected';
			}
	...
	public static function make_html_yearly_sales_Report_table($targetYear,$startMonth){
	...
	public function save_payment_history_ajax(Request $request){
		...
		$payments = [
			...
			'amount_payment' => mb_convert_kana($request->amount, "n"),
	...
	public static function make_html_reason_coming_cbox($targetSbj,$referee){
		...
		$htm_reason_coming_cbox.='●紹介者(氏名を入力してください。)</div><div class="col-auto"><input name="syokaisya_txt" id="syokaisya_txt" type="text" class="bg-white-500 border-solid pxtext-black rounded px-3 py-1" value="'.$referee.'"/></div>';
