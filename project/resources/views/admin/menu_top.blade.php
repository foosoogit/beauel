                ...
                <div class="card">
					...
					<div class="card-header">
						<div class="container">
							<div class="row">
								<div class="col-8">
									<span class="border border-success bg-success text-white rounded h5">&nbsp;{{session('target_branch_name')}}&nbsp;</span>
									メニュー&nbsp;{{config('app.name', 'Laravel')}}
									@if(session('target_branch_name')=="")
										<p class="display-1">店舗が選択されていません。</p>
									@endif
								</div>
								<div class="col-2">
                        ...
                        <ul>
							...
							<div class="row">
								...
								<div class="col-auto">
									<li>
										<form method="POST" action="{{route('admin.YearlyReport.post')}}">@csrf
											<button class="btn btn-primary btn-sm" type="submit">年報</button>&emsp;<select name="year">{!!$html_year_slct!!}</select> &nbsp;決算月<select name="kesan_month" onchange="save_kessan_month(this);"><option  value="0" >選択</option>{!!$htm_kesanMonth!!}</select>
										</form>
									</li>
								</div>
								{{-- 
								<div class="col-auto">
									<li>
										<form method="POST" action="{{route('admin.YearlyReport.post')}}">@csrf
											<button class="btn btn-primary btn-sm" type="submit">入金チェック</button>&emsp;<select name="year">{!!$html_year_slct!!}</select> &nbsp;決算月<select name="kesan_month" onchange="save_kessan_month(this);"><option  value="0" >選択</option>{!!$htm_kesanMonth!!}</select>
										</form>
									</li>
								</div>
								 --}}
							...
							<div class="col-auto">
								<li>
									<form method="POST" action="{{ route('admin.CashBookRep_download.post') }}">@csrf
										<button class="btn btn-primary btn-sm" type="submit">出納帳 印刷</button>&nbsp;&nbsp;<select name="year_start">{!!$html_year_slct!!}</select> <select name="month_start"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>～<select name="year_end">{!!$html_year_slct!!}</select> <select name="month_end"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>
									</form>
								</li>
							</div>
						<div class="row">
							<div class="col-auto">
								<li>
									<form method="POST" action="{{ route('admin.CashBookSmallList.post') }}">@csrf
										<button class="btn btn-primary btn-sm" type="submit">小口現金</button>
										<select name="year_start">{!!$html_year_slct!!}</select> <select name="month_start"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>～<select name="year_end">{!!$html_year_slct!!}</select> <select name="month_end"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>
									</form>
								</li>
							</div>
							{{--
							...
							<div class="col-auto">
								<li>
									<form method="POST" action="{{ route('admin.CashBookRep_download.post') }}">@csrf
										<button class="btn btn-primary btn-sm" type="submit" disabled>小口出納帳 印刷</button>&nbsp;&nbsp;<select name="year_start">{!!$html_year_slct!!}</select> <select name="month_start"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>～<select name="year_end">{!!$html_year_slct!!}</select> <select name="month_end"><option  value="0" >選択</option>{!!$html_month_slct!!}</select>
									</form>
								</li>
							</div>
							--}}
