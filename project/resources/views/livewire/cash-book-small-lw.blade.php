	...
	<div class="py-12">
		<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
			<div class="pb-4">
                ...
                <div class="mb-2 bg-info text-white"><span class="border border-success bg-success text-white rounded h5">&nbsp;{{session('target_branch_name')}}&nbsp;</span>&nbsp;&nbsp;出納帳</div>
				@if(session('target_branch_name')=="")
					<p class="display-1 text-danger">店舗が選択されていません。</p>
				@endif
                <div class="row pb-2">
                    <div class="col-auto">
                        <span class="font-weight-bold h4">全合計残金:  {{number_format($balance)}}  円</span>
                    </div>
                    {{-- 
                    <div class="col-auto">
                        <span class="font-weight-bold h4">{{session('target_branch_name')}}残金:  {{number_format($balance)}}  円</span>
                    </div>
                     --}}
                </div>
                <div class="row pb-2">
                    <div class="col-auto">
                        <button type="button" name="create_btn" id="create_btn" class="btn btn-primary btn-sm modalBtn text-left" 
                ...
                <div class="row pb-2">
                    <div class="form-check col-auto">
                        <input class="form-check-input" type="checkbox" value="payment" name="amount_type[]" id="payment_cbox" onclick="p_d_manage()" {{session('serch_payment_small_flg')}}  wire:click="serch_payment()">
                        <label class="form-check-label" for="payment_cbox">出金</label>
                    </div>
                    <div class="form-check col-auto">
                        <input class="form-check-input" type="checkbox" value="deposit" name="amount_type[]" id="deposit_cbox" onclick="p_d_manage()" {{session('serch_deposit_small_flg')}} wire:click="serch_deposit()">
                ...
                <div class="row pb-2 font-weight-bold h2">
                    店舗別検索データ:&nbsp;&nbsp;{{session('target_branch_name')}}
                    <div class="col-auto">
            ...
            <table id="table_responsive container-fluid" class="table-striped table-hover">
                ...
                <tbody>
                    @foreach ($CashBooksSmallQuery as $dCashBookQuery)
                        <tr>
	        ...
	        {{$CashBooksSmallQuery->appends(request()->query())->links('pagination::bootstrap-4')}}
        </div>    
            <!-- モーダル・ダイアログ -->
                ...
                <div class="modal-content">
                    ...
                    <form name="submitForm" id="submitForm">@csrf
                        <div class="modal-body">
                            ...
                            <div class="row p-2">
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="payment_deposit_rdo" id="payment" onclick="payment_deposit_rdo_manage(this);" wire:model="payment_deposit_rdo" value='payment'> 
                                        <label class="form-check-label" for="payment">出金</label>
                                    </div>
                                </div>
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="payment_deposit_rdo" id="deposit" onclick="payment_deposit_rdo_manage(this);" wire:model="payment_deposit_rdo" value='deposit'>
                                        <label class="form-check-label" for="deposit">入金</label>
                                    </div>
                                </div>
                                <div class="col-auto">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="payment_deposit_rdo" id="transfer" onclick="payment_deposit_rdo_manage(this);" wire:model="payment_deposit_rdo" value='deposit'>
                                        <label class="form-check-label" for="transfer">出納帳から入金</label>
                                    </div>
                                </div>
                                <span class="text-start text-danger" id="payment_transfer_rdo_for_error"></span>
                            ...
                            <div class="row p-2">
                                <div class="col-auto">
                                    <label>摘要：<input type="datalist" id="summary" name="summary" wire:model="summary"/></label>
